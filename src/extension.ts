...
import {NormalLWC}  from "./NormalLwc";
import { SUBLMS,PUBLMS,LMSChannel } from "./LMS";
import { Parent,CHILD } from "./ParentWithChild";
...
async function createOne(newname : string,result:string[]) {
	let options = {
		prompt: "Label: ",
	...
	if(newname !== 'MessageChannel'){
		...
		if(vscode.workspace.workspaceFolders !== undefined) {
			...
			if(newname === 'lwc'){
				let NormalLWC_temp=new NormalLWC();
		...
		//create a folder
		if(vscode.workspace.workspaceFolders !== undefined) {
			const wsPath = vscode.workspace.workspaceFolders[0].uri.path+'/force-app/main/default';// gets the path of the first workspace folder
			const wsedit = new vscode.WorkspaceEdit();
			const filePath = vscode.Uri.file(wsPath + '/messageChannels/'+itemname+'.messageChannel-meta.xml');
			vscode.window.showInformationMessage(filePath.toString());
			wsedit.createFile(filePath, { ignoreIfExists: true });
			
			let LMS_temp=new LMSChannel();
